...
class DemodulationConfig:
    ...
    SYNC_PATTERN = None  # 同步头模式 (全 1); None = 自动检测
    HEADER_PATTERN = [0, 1, 1, 1, 1, 1, 1, 0]  # 同步头比特模式：8 bits, 中间 6 个连续 1
    GAP_BITS = 34  # 两个同步头之间的比特数 = 1 (header trailing 0) + 32 (data) + 1 (header leading 0)

    # LED 区域检测
    LED_CHANNEL = "green"  # 使用的颜色通道
    COL_MARGIN_RATIO = 0.1  # LED 列边界向内收缩比例

    # 信号处理
    SMOOTHING_SIGMA = 1.5  # 行均值高斯平滑 sigma
    THRESHOLD_METHOD = (
        "polyfit"  # 二值化方法："otsu", "midpoint", "percentile", "polyfit"
    )
    POLYFIT_DEGREE = 3  # polyfit 二值化时多项式阶数（与 legacy/advisor 一致）
